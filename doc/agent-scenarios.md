...
## S24 · describe — ListTile with interactive trailing widget
...
## S25 · describe — plain tappable ListTile
...
- `INTERACTIVE:` contains `key=tappable_tile` with text containing
  `"Tappable tile"` (or similar — whatever `_extractDescribeText` collects)
- The entry has real on-screen coordinates (not `y=9999999`)
- No duplicate entries for `tappable_tile`

---

## S26 · describe — display-only ListTile is not surfaced
